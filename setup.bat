...
REM    1. Downloads / updates the library submodules (DigitalCert, Filesystem,
REM       DUF) to the exact versions this workspace expects.
...
if errorlevel 1 (
    ...
    echo         Clone COMManifestStudio with GitHub Desktop or "git clone",
...
if errorlevel 1 (
    ...
    echo           - https://github.com/NilsSve/Library-cFilesystem.git
...
echo === Setup complete ===
echo.
echo The Libraries\ folder now holds DigitalCert, Filesystem and DUF at the
echo versions this workspace expects. From now on a normal "git pull" (or
echo Pull in GitHub Desktop) will also update these libraries automatically.
echo.
